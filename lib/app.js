...
function socketFunction(redis_adapter) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('auth', function(message) {
            ...
            getSession(socket.roomname).setHost(socket, message);
        ...
        socket.on('token_renew', function(token) {
            getSession(socket.roomname).host.token = token;
        });
        ...
        socket.on('session_updated', function() {
            getSession(socket.roomname).update();
        });
        ...
        socket.on('text', function(message) {
            getSession(socket.roomname).broadcastAnnotations('text', message, this);
            getSession(socket.roomname).addAnnotation(message);
            // socket.broadcast.to(socket.roomname).emit('text', message);
