...
Participants.prototype.retrieve = function(socket_id) {
    global.logger.info(this.participants.length);
    ...
    for(var i = 0; i < this.participants.length; i++) {
        ...
        if(participant.uuid === participant_uuid) {
            ...
            if(participant.socket_id === socket_id) {
                ...
                global.logger.debug('Found duplicate uuid with different socket id. Removing ', participant.socket_id, participant_uuid);
...
Participants.prototype.add = function(socket_id, participant_uuid) {
    if(
        ...
        deferred.promise.then(function(resolve) {
            ...
            global.logger.info('Session:', self.session.roomname, 'Added participant. UUID:', participant_uuid, 'socket id', socket_id, 'Total participants:', self.participants.length);
...
Participants.prototype.disconnectAllParticipants = function() {
    global.logger.debug('disconnecting all participants', this.participants.length);
    for(var i = 0; i < this.participants.length; i++) {
...
Participants.prototype.remove = function(socket_id) {
    global.logger.info('Remove participant called. Removing participant with socket id', socket_id);
    if(socket_id && typeof socket_id === 'string') {
        ...
        global.logger.info('reject', typeof(reject));
