...
class Participants extends base.ApiCall{
    ...
    set participantUuid(participantUuid){
        this._participantUuid = JSON.parse(JSON.stringify(participantUuid));
    ...
    hasParticipantData(){
        ...
        // global.logger.info('has participant data', k, this._participantUuid);
        return k;
    }
    remove(){
        if(!this.hasParticipantData() ){
            global.logger.debug('Participant could not be removed. Participant UUID was ', this._participantUuid);
            return;
        }
        request.del(
            this.callback.options.url,
