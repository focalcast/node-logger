    ...
    var cbError = function(roomname, uuid) {
        return function(err, response, body) {
            if(defined(response) && response.hasOwnProperty('statusCode') && (response.statusCode === 503 || response.statusCode === 500 || response.statusCode === 401)) {
                ...
                global.logger.warn(`Session: ${roomname} -- Error removing participant ${uuid}`, err);
            }
        }
    };
...
Session.prototype.addUser = function(socket, process) {
    if(socket.handshake.query.recorder) {
        this.recorder = new Recorder(this, socket, socket.handshake.query.guid, process);
