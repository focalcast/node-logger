...
var Session = function(roomname) {
    ...
    redisPub.hset(this.getHash(), 'roomname', roomname, function(err, res) {
        if(err) {
            ...
            logger.info(res[self.roomhash]);
        }
    });
    logger.debug(typeof roomname);
...
Session.prototype.removeParticipant = function(socket) {
    var self = this;
    try {
        var participant_uuid = this.participants.retrieve(socket.id);
    }
    catch(err) {
...
Session.prototype.addParticipant = function(socket, data) {
    ...
    var success = function(body) {
        ...
        self.metrics.increment('participant_added');
        self.updateSessionInfo();
        try {
    ...
    var error = function(error, response) {
        if(typeof response !== 'undefined') {
            ...
            logger.error('Error adding participant: ', response.statusCode, response.statusMessage);
        }
        else {
            logger.error('Error adding participant');
        }
    };
    this.api.participant.add(this.roomname, data, success, error);
};
