...
Session.prototype.inviteParticipants = function(invitation){
    var success = function(response){
        logger.info("Success adding participnat", response);
    };

    var error = function(err){
        logger.error("Error inviting participant err", err);
    };
    logger.info(invitation);
    var session_usid = invitation.session_usid;
    logger.debug('inviting participants', invitation);
    for(var x = 0; x < invitation.guests.length; x++){
        var data = {
            email : invitation.guests[x],
            color : '#000000',
            invitation_status : 1,
            session_usid : session_usid
        };
        logger.debug('participant data', data);
        this.api.inviteSessionParticipant(session_usid, data).then(success, error);
    }
};
...
Session.prototype.addParticipant = function(socket, data){
    ...
    var error = function(error, response){
        if(typeof response !== 'undefined'){
            ...
            logger.error('Error adding participant: ' ,error, body, message, data);
        }else{
