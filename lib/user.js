...
class User {
    ...
    setListeners() {
        ...
        this.on('disconnect', (reason) => {
            ...
            if(defined(this.participant)) {
                this.session.removeParticipant(this.socket, this.participant);
            }
            else {
        ...
        this.on('add_user', (data) => {
            this.session.participants.add(this.socket, data).then((participant) => {
                this.participant = participant;
                this.socket.emit('deactivate_participant');
                this.session.update(true);
            });
        ...
        this.on('retrieve_annotations', (message) => {
            if(defined(this.session.presentation) && defined(this.session.presentation.annotations)) {
                this.session.presentation.annotations.retrieve(this.socket);
            }
            return;
        });
        ...
        this.on('force_slide', (slide) => {
            ...
            //    global.logger.debug('Forcing slide update');
            //    this.session.presentation.slide = slide;
            this.session.update(true);
        ...
        this.on('get_slide', () => {
            ...
            if(defined(this.session.presentation.slide)) {
                this.emit('set_slide', this.session.presentation.slide.url);
            }
        });
        ...
        this.on('text', (path) => {
            if(defined(this.session.presentation) && defined(this.session.presentation.annotations)) {
                this.session.presentation.annotations.broadcast('text', path, this.socket);
                this.session.presentation.annotations.add(path);
            }
        });

        this.on('path_part', (path) => {
            if(defined(this.session.presentation) && defined(this.session.presentation.annotations)) {
                this.session.presentation.annotations.broadcast('path_part', path, this.socket);
            }
        ...
        this.on('undo', () => {
            if(defined(this.session.presentation) && defined(this.session.presentation.annotations)) {
                this.session.presentation.annotations.undo();
            }
        });

        this.on('clear', () => {
            if(defined(this.session.presentation) && defined(this.session.presentation.annotations)) {
                this.session.presentation.annotations.clear();
            }
        });

        this.on('poll_response', (response)=>{
            this.session.host.send('poll_response', response);
